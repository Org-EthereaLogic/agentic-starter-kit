...
| --- | --- | --- | --- | --- |
| ASI-01 | Agent Goal Hijack — redirected goals via injected instructions or poisoned content | `CRIT-001`, `CRIT-002`, `CRIT-008`; `P1`, `P3` | `scripts/marker-scan.sh` (canonical-surface integrity); `scripts/check-governance.sh` (required-artifact integrity); `.githooks/pre-commit`+`.githooks/pre-merge-commit`+`.githooks/pre-push` (git-layer primary boundary — block protected-branch commits/pushes after shell resolution, so shell idioms cannot dodge them); `.claude/hooks/pre-tool-use.js` (agent-layer defense-in-depth — fast early block of protected-branch git actions) | Partial — canonical-surface and protected-branch guardrails landed; richer prompt-injection-specific controls expand in Phase 6 |
| ASI-02 | Tool Misuse and Exploitation — chained subcommands, manipulated tool outputs | `CRIT-008`, `IMP-004` | `.githooks/pre-commit`+`.githooks/pre-merge-commit`+`.githooks/pre-push` (git-layer primary boundary — enforce the protected-branch policy after the shell resolves the command, so eval/exec/`\git`/`git${IFS}push`/`bash -cl`/chains cannot bypass commit/merge/push; a conflict-free `cherry-pick`/`revert`/`rebase`/`am` runs no commit-time hook and so can land locally, backstopped by `pre-push` + the agent layer + server-side protection); `.claude/hooks/pre-tool-use.js` (agent-layer defense-in-depth — re-evaluates chained, nested-shell, `eval`, and `\git` git commands). Broader tool allowlists land in Phase B | Partial — protected-branch enforcement landed at the git layer; broader tool-policy enforcement lands in Phase B |
| ASI-03 | Identity and Privilege Abuse — exploiting delegated trust or inherited credentials | `CRIT-008`, `IMP-006` | `.githooks/pre-commit`+`.githooks/pre-merge-commit`+`.githooks/pre-push` (git-layer primary boundary — protected-branch push, refspec, broad-push, and commit gates enforced after shell resolution, incl. `sudo`/`doas`-wrapped git); `.claude/hooks/pre-tool-use.js` (agent-layer defense-in-depth); `scripts/check-action-pins.sh` (40-char SHA pin enforcement on every workflow `uses:`) | Covered |
...
| ASI-08 | Cascading Failures — false signals propagated through automated pipelines | `CRIT-005`, `CRIT-008`, `IMP-001` | `.githooks/pre-commit`+`.githooks/pre-merge-commit`+`.githooks/pre-push` (git-layer primary boundary) block forbidden protected-branch actions after shell resolution before they chain into git history, with `.claude/hooks/pre-tool-use.js` as agent-layer defense-in-depth; `.claude/hooks/{session-start,user-prompt-submit,post-tool-use}.cjs` append to `report/audit.jsonl` so propagated failures are forensically reconstructable. A dedicated cross-pipeline verification workflow lands in Phase 6 | Partial — guardrails + audit trail landed; verification workflow pending Phase 6 |
...
| ASI-10 | Rogue Agents — misalignment, concealment, self-directed action | `CRIT-008`; `P8` | `.githooks/pre-commit`+`.githooks/pre-merge-commit`+`.githooks/pre-push` (git-layer primary boundary) as the kill switch on forbidden protected-branch actions — enforced after shell resolution so shell idioms cannot dodge it — with `.claude/hooks/pre-tool-use.js` as agent-layer defense-in-depth; append-only audit trail at `report/audit.jsonl` written by the SessionStart, UserPromptSubmit, and PostToolUse hooks; risk-class autonomy ceiling in `CONSTITUTION.md §P8`. Honest scope: an operator-integrity control, not a sandbox (a user can unset `core.hooksPath`) | Covered |
...
- **Elevation of privilege** — covered by `CRIT-007`, `CRIT-008`
  (git-layer protected-branch hook `.githooks/pre-commit`+`pre-merge-commit`+`pre-push`
  as the primary boundary, with `.claude/hooks/pre-tool-use.js` as
  agent-layer defense-in-depth).
